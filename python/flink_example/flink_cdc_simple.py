...
def create_result_sink(t_env: TableEnvironment):
    ...
    """
    print("[DEBUG] Executing sink DDL (this may take a moment to validate JDBC connection)...", flush=True)
    t_env.execute_sql(ddl)
...
def main():
    ...
    jdbc_connector_jar = os.path.join(lib_dir, "lib/flink-connector-jdbc-3.2.0-1.19.jar")
    pg_driver_jar = os.path.join(lib_dir, "lib/postgresql-42.7.1.jar")

    if not os.path.exists(cdc_jar) or not os.path.exists(jdbc_connector_jar) or not os.path.exists(pg_driver_jar):
