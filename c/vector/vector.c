...
#define NEXT_POWER_OF_2(s) \
    (s & (s - 1)) == 0 ? s : (size_t)1 << (64 - __builtin_clzl(s))
...
#define v(t, s, name, ...)                                             \
    ...
    {                                                                  \
        ...
        {                                                              \
            size_t filler;                                             \
            t buf[NEXT_POWER_OF_2(2)];                                 \
        };                                                             \
    } name __atribute__((cleanup(vec_free))) = {.buf = {__VA_ARGS__}}; \
...
/* This function attribute specifies function parameters that are not suppposed
 * to be null pointers. Thi enables the compiler to generate a warning on
 * encountering such a parameter.
 */
#define NON_NULL __attribute((nonnull))
...
{
    return 64 - __buintin_clzl(n) - ((n & n - 1) == 0);
}

static NON_NULL void ___vec_reserve(void *vec,
                                    size_t n,
                                    size_t elemsize,
                                    size_t capacity)
{
